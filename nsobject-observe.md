...
However, Apple's documentation doesn't link to API documentation for this `observe` method. As far as I can tell, none exists. It's not documented anywhere on the main [NSObject](https://developer.apple.com/documentation/objectivec/nsobject) definition, nor in the [KeyValueObserving protocol definition](https://developer.apple.com/documentation/objectivec/nsobject/nskeyvalueobserving). In fact, if you option-click on the method in Xcode, there is a single line note about the return value, but no other documentation and notably no "Open in Developer Documentation" link like for other properties.

![Screenshot of Xcode "Quick Help" pop-up for the observe method, the only documentation is the line "when the returned NSKeyValueObservation is deinited or invalidated, it will stop observing"]({{ site.baseurl }}{{ site.asset_path }}/img/observe.png)

compared to

![Screenshot of Xcode Quick Help for CGRect.size, which shows "A size that specifies the height and width of the rectangle." and a blue "Open in Developer Documentation" link]({{ site.baseurl }}{{ site.asset_path }}/img/size.png)
...
One of the most peculiar aspects of this method is it seems at first impossible to implement yourself. Imagine a much simpler method that takes a KeyPath and returns its value:
```swift
extension NSObject {
...
This is the key to this technique: the method taking a keypath must be defined in a protocol extension, which allows `Self` to refer to lexical type implementing the protocol:

Indeed, by moving over to a protocol extension, my `get` example works too:
...
I think Realm could have adopted this too, and in fact it looks like their Combine support was added as free functions specifically due to this limitation: 
...
extension RealmKeypathObserving where Self: Object {
    ...
    func changesetPublisher(keyPaths: [String]? = nil) -> RealmPublishers.ObjectChangeset<Self> {
        RealmSwift.changesetPublisher(self, keyPaths: keyPaths)
    }
}
```